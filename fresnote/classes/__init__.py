...
#         self.allNotebooks = config.sections()
#
#         self.notebookName = notebookName 
#         self.notebookDir = config.get(self.notebookName, 'notebookDir')
#         if '~' in self.notebookDir:
#             self.notebookDir = os.path.expanduser(self.notebookDir)
#
#         # The existence of the following is checked with load_notebook_func
#         self.db = '{}/{}'.format(self.notebookDir, config.get(self.notebookName, 'notebookDB'))
#         self.docsDir = '{}/{}'.format(self.notebookDir, config.get(self.notebookName, 'docsDir'))
#
#
#
...
#
#
#     def convert_db_results_into_sections(self, results, order=[], info={}):
...
#
#
#     def add_new_project(self, projectTitle):
...
#                 """
#
#                 c.execute(query, ('First section', 'na', 'Section content', 1, 0, today))
...
#                 (project, chapter, sections)
#                 VALUES (?,?,?)
#                 """
#                 c.execute(query, (projectTitle, 'First chapter', json.dumps(sections)))
#                 conn.commit()
#
#         projectTitle = projectTitle.replace(' ', '_')
#         os.system("mkdir -p {}/{}".format(self.docsDir, projectTitle))
#
#
#
#     def add_new_chapter(self, projectTitle, chapterTitle):
#         today = datetime.today().strftime('%Y-%m-%d')
#         with contextlib.closing(sqlite3.connect(self.db)) as conn:
#             with contextlib.closing(conn.cursor()) as c:
#                 query = """
...
#                 VALUES (?,?,?)
#                 """
#                 c.execute(query, (projectTitle, chapterTitle, json.dumps(sections)))
#                 conn.commit()
#
#
...
#
#
#     def get_all_sections_for_chapter(self, project, chapter):
#         with contextlib.closing(sqlite3.connect(self.db)) as conn:
#             with contextlib.closing(conn.cursor()) as c:
#                 query = """
#                 SELECT sections FROM chapters 
...
#                     results = []
#
#         sections = self.convert_db_results_into_sections(results, order=sectionsIDs, info={'project':project, 'chapter':chapter})
#
#         return sections
#
#
